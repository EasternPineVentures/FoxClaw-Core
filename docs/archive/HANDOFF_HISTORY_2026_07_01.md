...
- FoxClaw Ledger V0 added as the local-only receipt/proof/history layer for coordination
  packets. The package is `foxclaw/ledger/`, the operator doc is
  `docs/ledger/foxclaw_ledger_v0.md`, and the tools are `tools\ledger_record_demo.py`,
  `tools\ledger_list_receipts.py`, `tools\ledger_verify_receipt.py`, and
  `tools\ledger_review_queue.py`. It stores JSONL receipts and review tasks under
  `runtime_logs/foxclaw_ledger/`, verifies stable hashes, records blocked `auto_publish`
  attempts as blocked, and makes no live CoinFox API calls.
...
- Use `docs\foxclaw_commands.md`, `python tools\foxclaw_commands.py`, or
  ...
  command window.
- Use `docs\ledger\foxclaw_ledger_v0.md` and `python tools\ledger_record_demo.py`
  when you need to prove coordination packets can become local hash-verified receipts.
